...
class PINNModel(nn.Module):
    def __init__(self):
        ...
        self.dense6 = nn.Linear(256, 5)  # Output: x and y position and velocities, mu
...
for epoch in range(num_epochs):
    ...
    if epoch % 500 == 0:
        ...
        plt.plot(list(range(1,len(mu_hist)+1)),mu_hist, 'r-')
        plt.plot(list(range(1,len(mu_hist)+1)), mu_act*np.ones(len(mu_hist)),'b--')
        plt.title('Predicted Drag')
        plt.legend(['Predicted Drag, Actual Drag'])
        plt.show()
...
plt.xlabel('x')
plt.ylabel('y')
plt.savefig('find_drag_pred.jpeg', format = 'jpeg')
plt.show()
...
plt.title('Predicted Drag')
plt.legend(['Predicted Drag','Actual Drag'])
plt.savefig('find_drag_drag.jpeg', format = 'jpeg')
plt.show()

print(f'Drag estimate: {mu}')
print(f'Actual value: {mu_act}')